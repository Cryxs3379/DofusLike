...
bool Entity::FORCE_DEBUG_CIRCLE = false;
...
Entity::Entity(sf::Vector2i startPosition, EntityType type) 
    ...
    m_entityShape.setRadius(FORCE_DEBUG_CIRCLE ? 16.0f : 8.0f);
    ...
    m_entityShape.setOrigin({m_entityShape.getRadius(), m_entityShape.getRadius()});
    
    // Si FORCE_DEBUG_CIRCLE está activo, usar solo el círculo
    if (FORCE_DEBUG_CIRCLE) {
        std::cout << "[Entity] FORCE_DEBUG_CIRCLE=true, usando círculo de radio 16px (sin sprites)" << std::endl;
        m_useSprite = false;
        return;
    }
